...
""",
    ...
    assert heading_run.font.color.rgb == RGBColor(0, 0, 0)
...
def test_double_escaped_latex_exports_as_professional_omml() -> None:
    data = convert_markdown_to_docx(
        r"""\\[
F \\mathrm{=} -\\frac{1}{2} \\mathrm{(}mg \\mathrm{+} m_动g\\mathrm{)} \\mathrm{+} f_3
\\]"""
    )
    report = inspect_docx(data)
    assert report["equation_count"] >= 1
    assert report["linear_latex_count"] == 0
    with zipfile.ZipFile(io.BytesIO(data)) as archive:
        assert b"<m:f>" in archive.read("word/document.xml")


def test_latex_fence_exports_as_professional_omml() -> None:
    data = convert_markdown_to_docx(
        r"""```latex
F=-\frac{1}{2}(mg+m_{\mathrm{动}}g)+f_3
```"""
    )
    report = inspect_docx(data)
    assert report["equation_count"] >= 1
