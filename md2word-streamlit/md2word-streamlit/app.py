...
if download and download["hash"] == content_hash:
    ...
    with success_col:
        ...
        st.success(f"文档已生成{formula_note}，均为专业格式。普通段落已套用 Word“正文”样式（宋体五号）。")
    with download_col:
        st.download_button(
            "下载 .docx",
            data=download["data"],
...
- 行内公式用 `$E=mc^2$`，独立公式用 `$$ ... $$`。
- 也支持 `\(...\)` 与 `\[...\]`，导出前会自动规范化。
- ` ```latex ` 代码块以及复制文本中常见的双重反斜杠也会自动识别为公式。
- 导出结果使用 Word 原生 OMML 专业格式，可双击编辑，无需再手动点击“专业格式”；复杂宏、自定义 LaTeX 宏包命令可能需要改写为标准 LaTeX。
